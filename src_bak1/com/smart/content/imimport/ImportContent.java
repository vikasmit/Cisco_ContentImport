...
public class ImportContent {
	...
	imUrlContentService="",
	destFilePathForAttachment="",	destFileForImage="",documentCreatedPath="",parserFileLocation="", imageFileLocation="";
			...
			if (fileAbsolutePath != null && !"".equals(fileAbsolutePath.trim())) {
				...
				if (xmlText != null && !"".equals(xmlText.trim())) {
					...
					if(!response){
						...
						copyImageFile(imageFileLocation, imageNameList);
					}
				}
		...
		try {
			if (contentXML != null && !"".equals(contentXML.trim())) {
				...
				// Handle the path for im library folder for all the images
					//IMG id=img_expSection_1 src="images/kms_dt_expand.gif
				destFileForImage.substring(destFileForImage.indexOf("library/"));
				String finalContentXML = handleFinalInputXml(contentXML).replaceAll(" src=\"","<img src=\"/library/images/"+ imageFileLocation.substring(imageFileLocation.indexOf("imageLoc/")+ "imageLoc/".length()));
				
				String finalContentXMLTest = finalContentXML.replaceAll("<IMG border=0", "").replaceAll("<IMG id=img_expSection_1", "").
						replaceAll("<IMG id=img_expSection_2", "").replaceAll("<IMG id=img_expSection_3", "").replaceAll("<IMG id=img_expSection_4", "").
						replaceAll("<IMG id=img_expSection_5", "").replaceAll("<IMG id=img_expSection_6", "").replaceAll("<IMG id=img_expSection_7", "").
						replaceAll("<IMG id=img_expSection_9", "").replaceAll("<IMG id=img_expSection_8", "");
				
				finalContentXML=finalContentXMLTest;
			...
			for (String fileName : attachmentFilesName) {
				try {
					...
					File source = new File(sourceAtachmentPath + fileName); // source
																			// for
					System.out.println("Copy>>"+source);														// attachment
		...
		String result="";
			...
			if (xmlText != null && !"".equals(xmlText.trim())){
			    ...
			    while ((lastIndex = xmlText.indexOf("<IMG src=\"", lastIndex)) != -1) {
			        ...
			        System.out.println(valueOfIMG);
			        
			        finalValueIMG = valueOfIMG.substring(valueOfIMG.lastIndexOf("/") +1).replaceAll("%", " ");
			        
			        finalValueIMG = valueOfIMG.substring(valueOfIMG.lastIndexOf("=") +1).replaceAll("%", " ");
			        
			        System.out.println(finalValueIMG);
			    ...
			    while ((lastIndex = xmlText.indexOf(" src=\"", lastIndex)) != -1) {
			        ...
			        finalValueIMG = valueOfIMG.substring(valueOfIMG.lastIndexOf("/") +1).replaceAll("%20", " ");
			        System.out.println(finalValueIMG);
			        }
			        
			        if(!valueOfIMG.contains("_")){
			        finalValueIMGwithSpcChar = valueOfIMG.substring(valueOfIMG.lastIndexOf("=") +1).replaceAll("%20", " ");
			        System.out.println(finalValueIMGwithSpcChar);
			        }
		...
		try {
			if (htmlText != null && !"".equals(htmlText.trim())) {
				...
				while ((lastIndex = htmlText.indexOf(startText, lastIndex)) != -1) {
					...
					trimString = replacementStr.substring(replacementStr.lastIndexOf("=")+1).replaceAll("%20", " ");
					System.out.println(trimString);
					}
