...
public class CreateContentInput {
	...
	private boolean writeFileContent(String fileAbsolutePath, String fileName) {
		...
		try {
			if (fileAbsolutePath != null && !"".equals(fileAbsolutePath.trim())) {
				...
				String finalVisibility =null;
				if(xmlText.indexOf("<VISIBILITY_GROUP>")>-1 && xmlText.indexOf("</VISIBILITY_GROUP>")>0){
				String visibiltyGroup = xmlText.substring(xmlText.indexOf("<VISIBILITY_GROUP>"), xmlText.indexOf("</VISIBILITY_GROUP>"));
				if(visibiltyGroup!=null && visibiltyGroup.length()>0 && visibiltyGroup.indexOf("[~!~")>-1)
				 finalVisibility = visibiltyGroup.substring(visibiltyGroup.indexOf("[~!~"));
				}
				
				if(finalVisibility!=null && finalVisibility.contains("~!~Internal~!~Self-Help~!~")){
	...
	public String handleRuntimeData(String xmlText) {
		...
		if(xmlText.lastIndexOf("CDATA[~!~")>-1 && xmlText.lastIndexOf("~!~]]></VISIBILITY_GROUP>")>0)
		visibility = xmlText.substring(xmlText.lastIndexOf("CDATA[~!~"), xmlText.lastIndexOf("~!~]]></VISIBILITY_GROUP>"));
		if(visibility==null)visibility="Internal";
		
		
		
		if(visibility.indexOf("Self-Help")>0){
			temp.add("SELF_HELP");
		}
		if(visibility.indexOf("Internal")>0){
			temp.add("INTERNAL");
		}
